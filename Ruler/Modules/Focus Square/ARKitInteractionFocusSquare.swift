...
/**
 ...
 - Tag: FocusSquare
 */
class ARKitInteractionFocusSquare: SCNNode {
    // MARK: - Types
    ...
    private var segments: [ARKitInteractionFocusSquare.Segment] = []
	...
	override init() {
        ...
        let c: Float = ARKitInteractionFocusSquare.thickness / 2 // correction to align lines perfectly
        ...
        positioningNode.simdScale = float3(ARKitInteractionFocusSquare.size * ARKitInteractionFocusSquare.scaleForClosedSquare)
	...
	private func performOpenAnimation() {
		...
		SCNTransaction.animationDuration = ARKitInteractionFocusSquare.animationDuration / 4
		...
		SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
		SCNTransaction.animationDuration = ARKitInteractionFocusSquare.animationDuration / 4
        positioningNode.simdScale = float3(ARKitInteractionFocusSquare.size)
		SCNTransaction.commit()
	}
	...
	private func performCloseAnimation(flash: Bool = false) {
		...
		SCNTransaction.begin()
		SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
		SCNTransaction.animationDuration = ARKitInteractionFocusSquare.animationDuration / 2
		...
		if flash {
			let waitAction = SCNAction.wait(duration: ARKitInteractionFocusSquare.animationDuration * 0.75)
			let fadeInAction = SCNAction.fadeOpacity(to: 0.25, duration: ARKitInteractionFocusSquare.animationDuration * 0.125)
			let fadeOutAction = SCNAction.fadeOpacity(to: 0.0, duration: ARKitInteractionFocusSquare.animationDuration * 0.125)
            fillPlane.runAction(SCNAction.sequence([waitAction, fadeInAction, fadeOutAction]))
			
			let flashSquareAction = flashAnimation(duration: ARKitInteractionFocusSquare.animationDuration * 0.25)
            for segment in segments {
    ...
    private func scaleAnimation(for keyPath: String) -> CAKeyframeAnimation {
        ...
        let size = ARKitInteractionFocusSquare.size
        let ts = ARKitInteractionFocusSquare.size * ARKitInteractionFocusSquare.scaleForClosedSquare
        ...
        scaleAnimation.duration = ARKitInteractionFocusSquare.animationDuration
    ...
    private lazy var fillPlane: SCNNode = {
        let correctionFactor = ARKitInteractionFocusSquare.thickness / 2 // correction to align lines perfectly
        let length = CGFloat(1.0 - ARKitInteractionFocusSquare.thickness * 2 + correctionFactor)
        ...
        material.diffuse.contents = ARKitInteractionFocusSquare.fillColor
        material.isDoubleSided = true
        material.ambient.contents = UIColor.black
        material.lightingModel = .constant
        material.emission.contents = ARKitInteractionFocusSquare.fillColor
...
extension ARKitInteractionFocusSquare.State: Equatable {
    static func ==(lhs: ARKitInteractionFocusSquare.State, rhs: ARKitInteractionFocusSquare.State) -> Bool {
        switch (lhs, rhs) {
