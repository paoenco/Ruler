...
class VirtualObjectARView: ARSCNView {
    ...
    struct ARKitIntegrationHitTestRay {
        var origin: float3
    ...
    struct ARKitIntegrationFeatureHitTestResult {
        var position: float3
    ...
    func worldPosition(fromScreenPosition position: CGPoint, objectPosition: float3?, infinitePlane: Bool = false) -> (position: float3, planeAnchor: ARPlaneAnchor?, isOnPlane: Bool)? {
         ...
         */
        let featureHitTestResult = arKitInteractionHitTestWithFeatures(position, coneOpeningAngleInDegrees: 18, minDistance: 0.2, maxDistance: 2.0).first
         ...
         */
        let unfilteredFeatureHitTestResults = arKitIntegrationHitTestWithFeatures(position)
    ...
    func hitTestRayFromScreenPosition(_ point: CGPoint) -> ARKitIntegrationHitTestRay? {
        ...
        return ARKitIntegrationHitTestRay(origin: cameraPos, direction: rayDirection)
    }
    ...
    func arKitInteractionHitTestWithFeatures(_ point: CGPoint, coneOpeningAngleInDegrees: Float, minDistance: Float = 0, maxDistance: Float = Float.greatestFiniteMagnitude, maxResults: Int = 1) -> [ARKitIntegrationFeatureHitTestResult] {
        ...
        let results = features.points.flatMap { featurePosition -> ARKitIntegrationFeatureHitTestResult? in
            ...
            return ARKitIntegrationFeatureHitTestResult(position: hitTestResult,
    ...
    func arKitIntegrationHitTestWithFeatures(_ point: CGPoint) -> [ARKitIntegrationFeatureHitTestResult] {
        ...
        let possibleResults = features.points.map { featurePosition in
            return ARKitIntegrationFeatureHitTestResult(featurePoint: featurePosition, ray: ray)
        }
...
extension VirtualObjectARView.ARKitIntegrationFeatureHitTestResult {
    ...
    init(featurePoint: float3, ray: VirtualObjectARView.ARKitIntegrationHitTestRay) {
