...
class VirtualObject: SCNReferenceNode {
    ...
    var modelName: String {
        return referenceURL.lastPathComponent.replacingOccurrences(of: ".scn", with: "").replacingOccurrences(of: ".dae", with: "")
    }
    ...
    func setPosition(_ newPosition: float3, relativeTo cameraTransform: matrix_float4x4, smoothMovement: Bool) {
        ...
        // Limit the distance of the object from the camera to a maximum of 20 meters.
        if simd_length(positionOffsetFromCamera) > 20 {
            positionOffsetFromCamera = simd_normalize(positionOffsetFromCamera)
            positionOffsetFromCamera *= 20
        }
